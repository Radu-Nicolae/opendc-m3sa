...
def plot_all_emissions(metamodels):
    ...
    plt.yticks(size=38)
    plt.ylabel("CO2 [kg]", size=38)
    plt.xlabel("Time over 1 month, granularity=300,000ms", size=28)
    plt.savefig("all_emissions.pdf")
    plt.show()
...
if __name__ == '__main__':
    ...
    print(
        f"Simulation with migration: {simulate_with_migration(metamodels=metamodels, migration_granularity=MS_GRANULARITY, starting_location=starting_location)[1]}.")

    plot_all_emissions(metamodels)
    # migrator = Migrator()
    # migrator.migrate()
